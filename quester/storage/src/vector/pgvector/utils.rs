...
use crate::{embedded_knowledge, semantic_knowledge, ActualDbPool, StorageError, StorageErrorKind, StorageResult};
                ...
                match score_query_result {
                    Ok(score) => {
                        if visited_pairs.insert((subject.clone(), object.clone())) {
                            combined_results.push((id, doc_id, subject.clone(), object.clone(), doc_source, sentence, event_id, score));
                            let mut new_conn = pool.get().await.map_err(|e| StorageError {
                                kind: StorageErrorKind::Internal,
                                source: Arc::new(anyhow::Error::from(e)),
                            })?;
                            Box::pin(traverse_node(pool, subject, combined_results, visited_pairs, &mut new_conn, depth + 1)).await?;
                        }
                    }
                    Err(e) => {
                ...
                match score_query_result {
                    Ok(score) => {
                        if visited_pairs.insert((subject.clone(), object.clone())) {
                            ...
                            Box::pin(traverse_node(pool, object, combined_results, visited_pairs, &mut new_conn, depth + 1)).await?;
                        }
                    }
                    Err(e) => {
                        eprintln!("Error querying score for event_id {}: {:?}", event_id, e);
...
pub fn extract_unique_pairs(
    ...
    for (_id, _doc_id, subject, object, _doc_source, _sentence, _event_id, _score) in traverser_results {
