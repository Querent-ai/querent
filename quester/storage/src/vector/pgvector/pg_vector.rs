...
impl Storage for PGVector {
	...
	async fn similarity_search_l2(
		...
		let query_result = embedded_knowledge::dsl::embedded_knowledge
			...
			.load::<(String, String, Option<Vector>, String, Option<String>)>(&mut *conn)
			.await;
		match query_result {
			Ok(result) => {
				let mut results = Vec::new();
				for (doc_id, knowledge, _embeddings, predicate, sentence) in result {
...
mod test {
	...
	async fn test_similarity_search_l2() {
		let _storage = setup_test_environment().await;
		...
		// TODO comment out the following code block when local testing
		// let test_vector = vec![
		// 	-0.05037749,
		// 	-0.0009511291,
		// 	0.12806408,
