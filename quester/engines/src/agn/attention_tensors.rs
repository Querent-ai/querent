...
use crate::{Engine, EngineResult, EngineError};
...
impl Engine for AttentionTensorsEngine {
    async fn process_ingested_tokens(
        ...
        while let Some(token) = token_stream.next().await {
            ...
            let cleaned_data: Vec<String> = token.data
                .into_iter()
        ...
        for chunk in &all_chunks {
            let tokenized_chunk = self.llm.tokenize(chunk).await.map_err(|e| EngineError::from(e))?;
        ...
        let classified_sentences = if !self.entities.is_empty() {
            ...
            let initial_classified_sentences = label_entities_in_sentences(&self.entities, &all_chunks);
            ...
            for tokens in &tokenized_chunks {
                let model_input = self.llm.model_input(tokens.clone()).await.map_err(|e| EngineError::from(e))?;
            ...
            for input in &model_inputs {
                let classification_result = self.llm.token_classification(input.clone(), None).await.map_err(|e| EngineError::from(e))?;
            ...
            for (chunk, classification) in all_chunks.iter().zip(classification_results.iter()) {
                ...
                let filtered_entities: Vec<(String, String)> = classification
                    .iter()
...
mod tests {
    ...
    async fn test_txt_ingestor() {
        ...
        let pinned_receiver_stream = Box::pin(receiver_stream) as Pin<Box<dyn Stream<Item = IngestedTokens> + Send>>;

        // Process the ingested tokens
        let mut engine_stream = engine.process_ingested_tokens(pinned_receiver_stream).await.unwrap();
