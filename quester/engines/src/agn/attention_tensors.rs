use crate::utils::{
	add_attention_to_classified_sentences, calculate_biased_sentence_embedding, create_binary_pairs, label_entities_in_sentences, match_entities_with_tokens, merge_similar_relations, remove_newlines, split_into_chunks, tokens_to_words, ClassifiedSentence, ClassifiedSentenceWithRelations
};
...
impl AttentionTensorsEngine {
	pub fn new(llm: Arc<dyn LLM>, entities: Vec<String>, sample_entities: Vec<String>, embedding_model: Option<TextEmbedding>) -> Self {
...
impl Engine for AttentionTensorsEngine {
    async fn process_ingested_tokens<'life0>(
        ...
        if self.embedding_model.is_none() {
            return Err(EngineError::new(EngineErrorKind::ModelError, anyhow::anyhow!("Embedding model not initialized").into()));
        }
        ...
        let stream = stream! {
            ...
            while let Some(token) = token_stream.next().await {
                ...
                println!("This is the token stream-------------{:?}", all_chunks);
                ...
                println!("tokeniuzed Chunks--------{:?}", tokenized_chunks);
                let classified_sentences = if !entities.is_empty() {
                    println!("User provided Entities-------------");
                ...
                for sentence_with_relations in all_sentences_with_relations {
                    ...
                    for head_tail_relation in &sentence_with_relations.relations {
                        for (predicate, _score) in &head_tail_relation.relations {
							...
							// println!("Event also ready to send -----{:?}", event);
                            yield Ok(event);
                        }
                    }
                    ...
                    for relation in &sentence_with_relations.relations {
                        ...
                        for (predicate, score) in &relation.relations {
                            ...
                            // println!("{:?}", biased_embedding);
                            let id = format!("{}-{}-{}", head_entity, predicate, tail_entity);
                            println!("This is the vector id {:?}", id);
...
mod tests {
	...
	async fn test_txt_ingestor() {
		...
		let engine = AttentionTensorsEngine::new(embedder, vec!["oil".to_string(), "gas".to_string(),"porosity".to_string(), "joel".to_string(), "india".to_string(),"microsoft".to_string(), "nitrogen gas".to_string()], vec!["oil".to_string(), "gas".to_string(),"porosity".to_string(), "joel".to_string(), "india".to_string(),"microsoft".to_string(), "nitrogen gas".to_string()], Some(embedding_model));
