	...
	S::Error: Into<BoxError>,
{
	...
	where
		...
		R: Send + 'static,
	{
		Self { bound, inner: TowerBuffer::new(service, bound) }
	}
}
	...
	S: Service<R>,
{
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		f.debug_struct("Buffer").field("bound", &self.bound).finish()
	}
}
...
{
	fn clone(&self) -> Self {
		Self { bound: self.bound, inner: self.inner.clone() }
	}
}
...
impl<R> BufferLayer<R> {
	pub fn new(bound: usize) -> Self {
		Self { bound, _phantom: PhantomData }
	}
}
...
mod tests {
	...
	async fn test_buffer_error() {
		...
		assert_eq!(service.ready().await.unwrap().call((10, 2)).await.unwrap(), 5);
		assert_eq!(
			service.ready().await.unwrap().call((10, 0)).await.unwrap_err(),
	...
	async fn test_buffer_closed() {
		...
		assert_eq!(service.ready().await.unwrap_err(), MyServiceError::Unavailable);
	}
}
