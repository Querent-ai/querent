...
use crate::transformers::bert::bert_model_functions::DTYPE;
use crate::transformers::roberta::roberta_model_functions::{RobertaModel as CandleRobertaModel, RobertaConfig };
...
impl EmbedderOptions {
    pub fn new() -> Self {
        Self { model: "roberta-base".to_string(), revision: None, distribution: None, local_dir: None }
    }
}
...
mod tests {
    ...
    async fn test_attention_tensor_to_2d_vector() {
        ...
        let token_classification_output = match embedder.token_classification(model_input.clone(), None).await {
            Ok(output) => output,
            Err(e) => {
