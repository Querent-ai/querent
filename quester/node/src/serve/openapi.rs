...
use crate::{cluster_api::ClusterApi, health_check_api::HealthCheckApi, DiscoveryApi, SemanticApi};
...
pub fn build_docs() -> utoipa::openapi::OpenApi {
			...
			utoipa::openapi::InfoBuilder::new()
				...
				.contact(Some(
					utoipa::openapi::ContactBuilder::new()
						.name(Some("Querent AI"))
						.email(Some("contact@querent.io"))
						.build(),
				))
				.build(),
		)
	...
	let tags = vec![Tag::new("Cluster Info"), Tag::new("Node Info")];
...
impl OpenApiMerger for utoipa::openapi::OpenApi {
	...
	fn with_path_prefix(mut self, prefix: &str) -> Self {
		...
		for (path, detail) in paths {
			...
			let adjusted_path =
				if path != "/" { format!("{prefix}{path}") } else { prefix.to_owned() };
...
mod openapi_schema_tests {
	...
	fn resolve_component_schemas(
		...
		if !pending_resolved.is_empty() || !failed_to_resolve.is_empty() {
			let errors_pending =
				pending_resolved.into_iter().map(|(path, _)| format!("{path:?}")).join("\n");
