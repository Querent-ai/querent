...
fn main() -> Result<(), Box<dyn std::error::Error>> {
	...
	let pyoxidize_packed_resource_path =
		format!("{}/build/{}/{}/resources/packed-resources", src_dir, target, profile);
	...
	if !Path::new(&pyoxidize_packed_resource_path).exists() {
		let output = Command::new("pyoxidizer")
			...
			.arg("--target-triple")
			.arg(&target)
			.args(if profile == "release" { Vec::from(["--release"]) } else { Vec::new() })
			.output()
	...
	if !Path::new(&pyembedded_dir).exists() {
		...
		// This is a hack, but I don't know how to pass flags to the child build.rs
	...
	let pyo3_config_alias = format!(
		"{}/{}-{}.txt",
		target_dir,
		Path::new(&pyo3_config_path).file_stem().unwrap().to_str().unwrap(),
		target,
	);
	...
	// configure pyoxidize
	println!("cargo:rustc-env=PYOXIDIZE_PYTHON_CONFIG_FILE={}", &pyoxidize_python_config_path);
