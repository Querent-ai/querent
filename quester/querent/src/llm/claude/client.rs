...
impl Claude {
	...
	async fn generate(&self, messages: &[Message]) -> Result<GenerateResult, LLMError> {
		...
		let res = match res.status().as_u16() {
			401 => Err(LLMError::AnthropicError(AnthropicError::AuthenticationError(
		...
		let generation = res.content.get(0).map(|c| c.text.clone()).unwrap_or_default();
	...
	fn build_payload(&self, messages: &[Message], stream: bool) -> Payload {
		let mut payload = Payload {
			...
			messages: messages.iter().map(|m| ClaudeMessage::from_message(m)).collect::<Vec<_>>(),
