...
",
		...
		let chain = LLMChainBuilder::new().prompt(prompt).llm(llm).build().unwrap(); //safe to unwrap
		self.llm = Some(chain);
		self
	}
	...
	pub async fn build(mut self) -> Result<RouteLayer, RouteLayerBuilderError> {
		...
		for (route, embedding) in
			self.routes.iter_mut().filter(|r| r.embedding.is_none()).zip(embeddings)
		{
