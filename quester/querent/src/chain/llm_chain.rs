...
impl LLMChainBuilder {
	pub fn new() -> Self {
		Self { prompt: None, llm: None, options: None, output_key: None }
	}
	...
	pub fn build(self) -> Result<LLMChain, ChainError> {
		...
		let mut llm =
			self.llm.ok_or_else(|| ChainError::MissingObject("LLM must be set".into()))?;
		...
		let chain =
			LLMChain { prompt, llm, output_key: self.output_key.unwrap_or("output".to_string()) };
...
impl Chain for LLMChain {
	...
	async fn invoke(&self, input_variables: PromptArgs) -> Result<String, ChainError> {
		let prompt = self.prompt.format_prompt(input_variables.clone())?;
		log::debug!("Prompt: {:?}", prompt);
		let output = self.llm.generate(&prompt.to_chat_messages()).await?.generation;
		Ok(output)
	}
...
mod tests {
	...
	async fn test_invoke_chain() {
		...
		assert!(result.is_ok(), "Error invoking LLMChain: {:?}", result.err())
	}
}
