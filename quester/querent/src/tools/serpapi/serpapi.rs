...
impl SerpApi {
	pub fn new(api_key: String) -> Self {
		Self { api_key, location: None, hl: None, gl: None, google_domain: None }
	}
	...
	pub async fn simple_search(&self, query: &str) -> Result<String, Box<dyn Error>> {
		let mut url =
			format!("https://serpapi.com/search.json?q={}&api_key={}", query, self.api_key);
...
fn get_answer_box(result: &Value) -> String {
	if let Some(map) = result["answer_box"].as_object() {
		...
		if let Some(snippet) = map.get("snippet_highlighted_words").and_then(|v| v.as_array()) {
			if !snippet.is_empty() {
...
mod tests {
	...
	async fn serpapi_tool() {
		...
		let s = serpapi.simple_search("Who is the President of Peru").await.unwrap();
		println!("{}", s);
	}
}
