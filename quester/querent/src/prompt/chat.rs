...
impl MessageFormatterStruct {
	...
	pub fn add_messages_placeholder(&mut self, placeholder: &str) {
		self.items.push(MessageOrTemplate::MessagesPlaceholder(placeholder.to_string()));
	}
...
mod tests {
	...
	fn test_message_formatter_macro() {
		...
		let formatted_messages =
			formatter.format_prompt(input_variables).unwrap().to_chat_messages();
		...
		assert_eq!(formatted_messages[1].content, "AI response: This is a test test2");
