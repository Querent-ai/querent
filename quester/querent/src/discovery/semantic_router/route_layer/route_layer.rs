	...
	pub fn aggregate(&self, values: &[f64]) -> f64 {
		match self {
			...
			AggregationMethod::Max =>
				*values.iter().max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap_or(&0.0),
		}
	}
}
...
impl RouteLayer {
	...
	pub async fn call<S: Into<String>>(
		...
		let tool_input =
			self.generate_tool_input(&query, &router.tool_description.unwrap()).await?;

		Ok(route_choise.map(|route| RouteChoise { tool_input: Some(tool_input), ..route }))
	}
	...
	pub async fn call_embedding(
		...
		let mut scores_by_route: HashMap<String, Vec<f64>> = HashMap::new();
		for (route_name, score) in &similar_routes {
			scores_by_route.entry(route_name.clone()).or_default().push(*score);
		}
...
mod tests {
	...
	async fn test_route_layer_builder() {
		let captial_route = Router::new(
			"captial",
			&["Capital of France is Paris.", "What is the captial of France?"],
		);
	...
	"Input should be a search query."#,
		...
		let weather_route = Router::new(
			"temperature",
			&["What is the temperature?", "Is it raining?", "Is it cloudy?"],
		)
		...
		let router_layer = RouteLayerBuilder::default()
			...
			.await
			.unwrap();
		let routes = router_layer.call("What is the temperature in Peru?").await.unwrap();
