...
use storage::utils::{get_top_k_pairs, extract_unique_pairs, find_intersection};
                ...
                for storage in storages.iter() {
                    ...
                    match search_results {
                        Ok(results) => {
                            ...
                            if self.previous_query_results.is_empty()  || message.session_id != self.previous_session_id {
                                match &traverser_results_1 {
                                    Ok(ref traverser_results) => {
                                        self.previous_query_results = serde_json::to_string(traverser_results).unwrap_or_default();
                                ...
                                if let Ok(ref traverser_results) = traverser_results_1 {
                                    ...
                                    tokio::spawn(insert_discovered_knowledge_async(storage.clone(), document_payloads.clone()));
                                } else {
                                    log::error!("Failed to search for similar documents in traverser: {:?}", traverser_results_1.unwrap_err());
                                }
                            } else {
                                // If previous_query_results is not empty, parse and combine with current results
                                let previous_results: Vec<(i32, String, String, String, String, String, String, f32)> =
                                    serde_json::from_str(&self.previous_query_results).unwrap_or_default();
                                ...
                                let formatted_output_2 = extract_unique_pairs(previous_results.clone(), self.previous_filtered_results.clone());
                                ...
                                match final_traverser_results.clone() {
                                    Ok(ref results) => {
                                        self.previous_query_results = serde_json::to_string(results).unwrap_or_default();
                                    },
                                    Err(e) => {
                                        log::error!("Failed to search for similar documents in traverser: {:?}", e);
                                    }
                                ...
                                if let Ok(ref traverser_results) = final_traverser_results {
                                    ...
                                    log::error!("Failed to search for similar documents in traverser: {:?}", final_traverser_results.unwrap_err());
                                }
                            }
                        },
                        Err(e) => {
