...
impl Chain for ConversationalChain {
	async fn call(&self, input_variables: PromptArgs) -> Result<GenerateResult, ChainError> {
		...
		memory.add_message(Message::new_ai_message(&input_variables[DEFAULT_INPUT_VARIABLE]));
...
mod tests {
	...
	async fn test_invoke_conversational() {
		...
		assert!(result_first.is_ok(), "Error invoking LLMChain: {:?}", result_first.err());
		...
		assert!(result_second.is_ok(), "Error invoking LLMChain: {:?}", result_second.err());
