	...
	A: Agent,
{
	pub fn from_agent(agent: A) -> Self {
		Self { agent, max_iterations: Some(10), break_if_error: false, memory: None }
	}
	...
	A: Agent + Send + Sync,
{
	async fn call(&self, input_variables: PromptArgs) -> Result<GenerateResult, ChainError> {
		...
		if let Some(memory) = &self.memory {
			...
			input_variables
				.insert("chat_history".to_string(), json!(SimpleMemory::new().messages()));
		}

		loop {
			let agent_event =
				self.agent.plan(&steps, input_variables.clone()).await.map_err(|e| {
			...
			match agent_event {
				...
				AgentEvent::Finish(finish) => {
					...
					return Ok(GenerateResult { generation: finish.output, ..Default::default() });
				},
			}
