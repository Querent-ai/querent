...
impl<C: Config> OpenAiEmbedder<C> {
	pub fn new(config: C) -> Self {
		OpenAiEmbedder { config, model: String::from("text-embedding-ada-002") }
	}
...
impl<C: Config + Send + Sync> Embedder for OpenAiEmbedder<C> {
	async fn embed_documents(&self, documents: &[String]) -> Result<Vec<Vec<f64>>, EmbedderError> {
		...
		let embeddings = response
			...
			.map(|embedding| embedding.into_iter().map(|x| x as f64).collect::<Vec<f64>>())
			.collect();
	...
	async fn embed_query(&self, text: &str) -> Result<Vec<f64>, EmbedderError> {
		...
		let request =
			CreateEmbeddingRequestArgs::default().model(&self.model).input(text).build()?;

		let mut response = client.embeddings().create(request).await?;

		let item = response.data.swap_remove(0);

		Ok(item.embedding.into_iter().map(|x| x as f64).collect::<Vec<f64>>())
	}
}
