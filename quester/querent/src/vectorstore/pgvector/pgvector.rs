...
impl HNSWIndex {
	pub fn new(m: i32, ef_construction: i32, distance_function: &str) -> Self {
		HNSWIndex { m, ef_construction, distance_function: distance_function.into() }
	}
}
...
impl Store {
	...
	async fn drop_tables(&self) -> Result<(), Box<dyn Error>> {
		sqlx::query(&format!(r#"DROP TABLE IF EXISTS {}"#, self.embedder_table_name))
			.execute(&self.pool)
			.await?;

		sqlx::query(&format!(r#"DROP TABLE IF EXISTS {}"#, self.collection_table_name))
			.execute(&self.pool)
			.await?;
...
(uuid, document, embedding, cmetadata, collection_id) VALUES ($1, $2, $3, $4, $5)"#,
	...
	async fn similarity_search(
		...
		let rows = sqlx::query(&sql)
			...
			.bind(&Vector::from(query_vector.into_iter().map(|x| x as f32).collect::<Vec<f32>>()))
			.bind(limit as i32)
			.fetch_all(&self.pool)
			.await?;
		...
		let docs = rows
			...
			.map(|row| {
				...
				Ok(Document { page_content, metadata, score })
			})
