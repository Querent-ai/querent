...
impl StoreBuilder {
		...
		match &self.pool {
			...
			None => {
				let connection_url = match self.connection_url {
					...
					_ => env::var("PGVECTOR_CONNECTION_STRING").map_err(|_| {
						"PGVECTOR_CONNECTION_STRING environment variable not set, and no connection URL provided."
					})?,
				};
	...
	async fn create_or_get_collection(
		...
		let row = sqlx::query(&sql).bind(&self.collection_name).fetch_one(&mut **tx).await?;
	...
	async fn remove_collection(
		...
		sqlx::query(&format!("DELETE FROM {} WHERE name = $1", self.collection_table_name))
			.bind(&self.collection_name)
			.execute(&mut **tx)
			.await?;
	...
	async fn create_collection_table_if_not_exists(
		...
		sqlx::query(&create_extension_sql).execute(&mut **tx).await?;
