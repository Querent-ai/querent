...
const QUERENT_ERROR_HEADER_NAME: &str = "querent-error-bin";
...
/// in the gRPC header [`QUERENT_ERROR_HEADER_NAME`]. This allows for propagating them transparently
	...
	E: GrpcServiceError,
{
	...
	match encode_error(&service_error) {
		Ok(header_value) => {
			status.metadata_mut().insert_bin(QUERENT_ERROR_HEADER_NAME, header_value);
		},
		Err(error) => {
...
where
	E: GrpcServiceError,
{
	if let Some(header_value) = status.metadata().get_bin(QUERENT_ERROR_HEADER_NAME) {
...
pub fn convert_to_grpc_result<T, E: GrpcServiceError>(
	...
	result.map(tonic::Response::new).map_err(|error| error.into_grpc_status())
}
