...
function SearchView() {
  ...
  const runSearch = (updatedSearchRequest: SearchRequest) => {
    ...
    questerClient.search(updatedSearchRequest).then((response) => {
  ...
  useEffect(() => {
    ...
    questerClient.getIndex(searchRequest.indexId).then((fetchedIndex) => {
      setIndex(fetchedIndex);
    });
  }, [searchRequest, questerClient, index]);
