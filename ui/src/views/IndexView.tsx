...
function IndexView() {
  ...
  const fetchIndex = useCallback(() => {
    ...
    if (pipelineId === undefined) {
      console.warn("`pipelineId` should always be set.");
      return;
    } else {
      questerClient.getIndex(pipelineId).then(
        (fetchedIndex) => {
    ...
    <ViewUnderAppBarBox>
      <FullBoxContainer>
        <QBreadcrumbs aria-label="breadcrumb">
          ...
          <Typography color="text.primary">{pipelineId}</Typography>
        </QBreadcrumbs>
        { renderFetchIndexResult() }
      </FullBoxContainer>
      { ApiUrlFooter('api/v1/indexes/' + pipelineId) }
    </ViewUnderAppBarBox>
  );
}
