...
impl TextElementGroup {
	pub fn text(&self) -> String {
		let texts = self.children.iter().map(|x| x.text().to_owned()).collect::<Vec<String>>();
		texts.join("\n")
	}

	pub fn bounds(&self) -> Rect {
		let rects = self.children.iter().map(|x| x.bounds().to_owned()).collect::<Vec<Rect>>();
		Rect::union(&rects)
	}

	pub fn font_size(&self) -> f32 {
		let values = self.children.iter().map(|x| x.font_size()).collect::<Vec<f32>>();
	...
	pub fn element_count(&self) -> usize {
		self.children.iter().map(|x| x.elements().len()).sum::<usize>()
	}
