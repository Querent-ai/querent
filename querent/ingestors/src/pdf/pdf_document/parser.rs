			...
			match group.kind() {
				...
				TextElementGroupKind::Paragraph => {
					...
					nodes.push(Node::Paragraph(
						Paragraph::builder().children(children).position(Some(position)).build(),
					));
					last_position.add_line();
				},
				_ => {
...
fn create_position_with_children(children: &Vec<Node>) -> Position {
	let positional_nodes = children.iter().filter(|x| x.is_positioal()).collect::<Vec<&Node>>();
	let start = positional_nodes.first().unwrap().position().unwrap().start().to_owned();
	let end = positional_nodes.last().unwrap().position().unwrap().end().to_owned();
