...
impl Document {
	...
	pub fn all_texts(&self) -> Vec<String> {
		self.nodes.par_iter().map(|x| extract_node_content(x)).collect::<Vec<String>>()
	}
...
fn extract_node_content(node: &Node) -> String {
	...
	match node {
		...
		Node::Paragraph(paragraph) => {
			...
			let texts =
				paragraph.children().iter().map(extract_node_content).collect::<Vec<String>>();
			buf.push_str(&texts.join(""));
		},
		...
		Node::List(list) => {
			...
			let texts = list.children().iter().map(extract_node_content).collect::<Vec<String>>();
			buf.push_str(&texts.join("\n"));
		},

		Node::ListItem(list_item) => {
			let texts =
				list_item.children().iter().map(extract_node_content).collect::<Vec<String>>();
			buf.push_str(&texts.join(" "));
		},
		...
		Node::Table(table) => {
			...
			let texts = table.children().iter().map(extract_node_content).collect::<Vec<String>>();
			buf.push_str(&texts.join(""));
		},

		Node::TableRow(table_row) => {
			let texts =
				table_row.children().iter().map(extract_node_content).collect::<Vec<String>>();
		...
		Node::TableCell(table_cell) => {
			let texts =
				table_cell.children().iter().map(extract_node_content).collect::<Vec<String>>();
			buf.push_str(&texts.join(" "));
		},
		Node::Heading(heading) => {
			...
			let texts =
				heading.children().iter().map(extract_node_content).collect::<Vec<String>>();
			buf.push_str(&texts.join(""));
		},
		Node::LineBreak => {
			buf.push_str("\n");
