...
impl ProtoGenerator {
	pub fn run(mut args: CodegenBuilder) -> anyhow::Result<()> {
		let service_generator = Box::new(QuerentServiceGenerator::new(
...
impl CodegenBuilder {
	...
	pub fn run(self) -> anyhow::Result<()> {
		...
		ProtoGenerator::run(self)
	}
}

struct QuerentServiceGenerator {
	result_type_path: String,
	error_type_path: String,
	generate_extra_service_methods: bool,
	generate_prom_labels_for_requests: bool,
...
impl ServiceGenerator for QuerentServiceGenerator {
...
impl CodegenContext {
	fn from_service(
		...
		let stream_type_alias = if service.methods.iter().any(|method| method.server_streaming) {
			quote! {
				pub type #stream_type<T> = common::ServiceStream<#result_type<T>>;
			}
		} else {
			TokenStream::new()
		};
		...
		let mailbox_name = quote::format_ident!("{}MessageBus", service.name);
...
impl SynMethod {
	fn request_type(&self, mock: bool) -> TokenStream {
		...
		if self.client_streaming {
			quote! { common::ServiceStream<#request_type> }
		} else {
			request_type
		}
	}
...
fn generate_service_trait(context: &CodegenContext) -> TokenStream {
	...
	let extra_trait_methods = if context.generate_extra_service_methods {
		quote! {
			...
			fn endpoints(&self) -> Vec<common::uri::Uri>;
		}
	} else {
		TokenStream::new()
	};
...
fn generate_client(context: &CodegenContext) -> TokenStream {
	...
	quote! {
		...
		impl #client_name {
			...
			pub fn from_balance_channel(balance_channel: common::tower::BalanceChannel<std::net::SocketAddr>, max_message_size: bytesize::ByteSize) -> #client_name
			{
...
fn generate_tower_svc_stack(context: &CodegenContext) -> TokenStream {
	...
	let tower_svc_stack_clone_impl = generate_tower_svc_stack_clone_impl(context);
...
fn generate_tower_svc_stack_attributes(context: &CodegenContext) -> TokenStream {
	...
	for syn_method in &context.methods {
		...
		let attribute = quote! {
			#attribute_name: common::tower::BoxService<#request_type, #response_type, #error_type>,
		};
...
fn generate_tower_svc_stack_clone_impl(context: &CodegenContext) -> TokenStream {
	let tower_svc_stack_name = &context.tower_svc_stack_name;
	...
	quote! {
		impl Clone for #tower_svc_stack_name {
			fn clone(&self) -> Self {
				Self {
...
fn generate_layer_stack_types_and_attributes(
	...
	for syn_method in &context.methods {
		...
		let type_alias = quote! {
			type #type_alias_name = common::tower::BoxLayer<common::tower::BoxService<#request_type, #response_type, #error_type>, #request_type, #response_type, #error_type>;
		};
		let attribute = quote! {
...
fn generate_layer_stack_impl(context: &CodegenContext) -> TokenStream {
	...
	for syn_method in &context.methods {
		...
		let shared_layer_method_bound = quote! {
			L: tower::Layer<common::tower::BoxService<#request_type, #response_type, #error_type>> + Clone + Send + Sync + 'static,
			<L as tower::Layer<common::tower::BoxService<#request_type, #response_type, #error_type>>>::Service: tower::Service<#request_type, Response = #response_type, Error = #error_type> + Clone + Send + Sync + 'static,
			<<L as tower::Layer<common::tower::BoxService<#request_type, #response_type, #error_type>>>::Service as tower::Service<#request_type>>::Future: Send + 'static,
		};
		let layer_method_bound = quote! {
			L: tower::Layer<common::tower::BoxService<#request_type, #response_type, #error_type>> + Send + Sync + 'static,
		...
		let layer_method_statement = quote! {
			self.#layer_attribute_name.push(common::tower::BoxLayer::new(layer.clone()));
		};
		let layer_method = quote! {
			...
			where
				#layer_method_bound
			{
				self.#layer_attribute_name.push(common::tower::BoxLayer::new(layer));
				self
			}
		};
		...
		let svc_statement = quote! {
			let #svc_attribute_name = self.#layer_attribute_name.into_iter().rev().fold(common::tower::BoxService::new(boxed_instance.clone()), |svc, layer| layer.layer(svc));
		};
	...
	quote! {
		impl #tower_layer_stack_name {
			...
			{
				self.build_from_boxed(Box::new(#client_name::from_channel(addr, channel, max_message_size)))
			}

			pub fn build_from_balance_channel(self, balance_channel: common::tower::BalanceChannel<std::net::SocketAddr>, max_message_size: bytesize::ByteSize) -> #client_name
			{
				self.build_from_boxed(Box::new(#client_name::from_balance_channel(balance_channel, max_message_size)))
			}

			pub fn build_from_mailbox<A>(self, mailbox: actors::MessageBus<A>) -> #client_name
			where
				A: actors::Actor + std::fmt::Debug + Send + 'static,
				#mailbox_name<A>: #service_name,
			{
...
fn generate_tower_mailbox(context: &CodegenContext) -> TokenStream {
	...
	let extra_mailbox_methods = if context.generate_extra_service_methods {
		quote! {
			...
			fn endpoints(&self) -> Vec<common::uri::Uri> {
				vec![common::uri::Uri::from_str(&format!("actor://localhost/{}", self.inner.actor_instance_id())).expect("URI should be valid")]
			}
		}
	} else {
		TokenStream::new()
	...
	quote! {
		...
		impl<A, E> std::ops::Deref for MailboxAdapter<A, E> where A: actors::Actor {
		...
		#[derive(Debug)]
		pub struct #mailbox_name<A: actors::Actor> {
			inner: MailboxAdapter<A, #error_type>
		}

		impl <A: actors::Actor> #mailbox_name<A> {
		...
		impl <A: actors::Actor> Clone for #mailbox_name<A> {
			fn clone(&self) -> Self {
				let inner = MailboxAdapter {
					inner: self.inner.clone(),
					phantom: std::marker::PhantomData,
		...
		where
			...
			#error_type: From<actors::AskError<E>>,
		{
			type Response = T;
			type Error = #error_type;
			type Future = BoxFuture<Self::Response, Self::Error>;
		...
		quote! {
			...
			fn endpoints(&self) -> Vec<common::uri::Uri> {
				self.connection_addrs_rx
					.borrow()
					.iter()
					.flat_map(|addr| common::uri::Uri::from_str(&format!("grpc://{addr}/{}.{}", #grpc_client_package_name_string, #service_name_string)))
					.collect()
			}
		}
	} else {
			...
			quote! { |response|
				{
					...
					let stream = common::ServiceStream::from(streaming);
...
fn generate_grpc_server_adapter_methods(context: &CodegenContext) -> TokenStream {
	...
	for syn_method in &context.methods {
			...
			quote! {
				{
					...
					common::ServiceStream::from(streaming)
				}
			}
		} else {
		...
		let associated_type = if syn_method.server_streaming {
			...
			quote! { type #associated_type_name = common::ServiceStream<tonic::Result<#response_type>>; }
		} else {
			TokenStream::new()
		};
		let into_response_type = if syn_method.server_streaming {
...
impl WithSuffixServiceGenerator {
	fn new(suffix: &str, service_generator: Box<dyn ServiceGenerator>) -> Self {
		Self { suffix: suffix.to_string(), inner: service_generator }
	}
}
