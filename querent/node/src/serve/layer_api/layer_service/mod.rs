...
pub type InsightsPool = Pool<SocketAddr, LayerServiceClient>;

pub type Result<T> = std::result::Result<T, LayerError>;

pub async fn start_layer_service(
...
) -> anyhow::Result<Arc<dyn LayerService>> {
	let licence_key = secret_store
		.get_rian_api_key()
		.await
		.map_err(|e| anyhow::anyhow!("Failed to get licence key: {:?}", e))?;
	let layer_agent_service = LayerAgentService::new(
		node_config.node_id.clone(),
		cluster.clone(),
		event_storages.clone(),
		licence_key.clone(),
	);
	let (layer_service_mailbox, _) = querent.spawn_builder().spawn(layer_agent_service);
	info!("Starting layer agent service");
	let layer_service = Arc::new(service::LayerImpl::new(
		event_storages,
		index_storages,
		metadata_store,
		layer_service_mailbox,
	));
	Ok(layer_service)
}
