...
impl common::ServiceError for LayerError {
...
impl From<LayerError> for tonic::Status {
	fn from(error: LayerError) -> Self {
		grpc_error_to_grpc_status(error)
	}
}

impl From<anyhow::Error> for LayerError {
	fn from(any_error: anyhow::Error) -> Self {
		LayerError::Internal(any_error.to_string())
	}
}

impl From<JoinError> for LayerError {
	fn from(join_error: JoinError) -> LayerError {
		LayerError::Internal(format!("spawned task in root join failed: {join_error}"))
	}
}

impl From<std::convert::Infallible> for LayerError {
	fn from(infallible: std::convert::Infallible) -> LayerError {
		match infallible {}
	}
}
