	...
	T: Rate,
{
	...
	pub fn new(inner: S, rate: T) -> Self {
		...
		let state = State::Ready { deadline, credit: rate.work() };
	...
	T: Rate,
{
	...
	fn poll_ready(&mut self, cx: &mut Context<'_>) -> Poll<Result<(), Self::Error>> {
		...
		if deposit >= debit {
			...
			self.state = State::Limited { debit: debit - deposit };
			self.sleep.as_mut().reset(Instant::now() + self.rate.period());
			Poll::Pending
		}
	}
	...
	fn call(&mut self, request: R) -> Self::Future {
		match self.state {
			State::Ready { mut deadline, mut credit } => {
				let now = Instant::now();
...
mod tests {
	...
	use super::*;
	use crate::tower::{buffer::BufferError, BufferLayer, ConstantRate};
	...
	impl RateMeter {
		fn new() -> Self {
			Self { cumulated_work: Arc::new(AtomicU64::new(0)) }
		}
	}
	...
	impl Service<Request> for RateMeter {
		...
		fn call(&mut self, request: Request) -> Self::Future {
			self.cumulated_work.fetch_add(request.cost, Ordering::Relaxed);
	...
	async fn test_rate_limit_over_multiple_periods() {
		...
		let now = Instant::now();
		service.ready().await.unwrap().call(Request { cost: 1 }).await.unwrap();
	...
	async fn test_rate_limit() {
		...
		let futures = (0..5).map(|_| {
			...
			tokio::time::timeout(Duration::from_millis(deadline), async move {
				loop {
					service.ready().await.unwrap().call(Request::random()).await.unwrap();
				}
			})
		});
