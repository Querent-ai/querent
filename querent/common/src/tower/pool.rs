...
{
	fn default() -> Self {
		Self { pool: Arc::new(RwLock::new(HashMap::default())) }
	}
}
	...
	V: Clone + Send + Sync + 'static,
{
	...
	pub fn is_empty(&self) -> bool {
		self.pool.read().expect("lock should not be poisoned").is_empty()
	}
	...
	where
		...
		K: Borrow<Q>,
	{
		self.pool.read().expect("lock should not be poisoned").contains_key(key)
	}
	...
	where
		...
		K: Borrow<Q>,
	{
		self.pool.read().expect("lock should not be poisoned").get(key).cloned()
	}
	...
	pub fn insert(&self, key: K, service: V) {
		self.pool.write().expect("lock should not be poisoned").insert(key, service);
	}

	/// Removes a value from the pool.
	pub fn remove(&self, key: &K) {
		self.pool.write().expect("lock should not be poisoned").remove(key);
	}
}
	...
	K: Eq + PartialEq + Hash,
{
	...
	{
		Self { pool: Arc::new(RwLock::new(HashMap::from_iter(iter))) }
	}
}
