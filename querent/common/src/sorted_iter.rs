...
pub trait SortedIterator: Iterator + Sized {
	...
	{
		DiffIterator { left: self.peekable(), right: other.peekable() }
	}
}
...
pub trait SortedByKeyIterator<K, V>: Iterator + Sized {
	...
	{
		DiffByKeyIterator { left: self.peekable(), right: other.peekable() }
	}
}
			...
			(Some((left_key, _)), Some((right_key, _))) => match left_key.cmp(right_key) {
				Ordering::Less => {
					let (left_key, left_value) =
						self.left.next().expect("The left iterator should not be empty.");
				...
				Ordering::Equal => {
					let (left_key, left_value) =
						self.left.next().expect("The left iterator should not be empty.");
					let (_, right_value) =
						self.right.next().expect("The right iterator should not be empty.");
	...
	fn test_diff_by_key() {
		...
		}
		{
			...
			let right: BTreeMap<_, _> =
				vec![(2, "b"), (4, "d"), (5, "e"), (6, "f")].into_iter().collect();
			let key_diff: Vec<_> = left.iter().diff_by_key(right.iter()).collect();
			assert_eq!(
				key_diff,
				vec![
