...
impl HostAddr {
	...
	pub fn parse_with_default_port(host_addr: &str, default_port: u16) -> anyhow::Result<Self> {
		if let Ok(socket_addr) = host_addr.parse::<SocketAddr>() {
			return Ok(Self { host: Host::IpAddr(socket_addr.ip()), port: socket_addr.port() });
		}
		if let Ok(ip_addr) = host_addr.parse::<IpAddr>() {
			return Ok(Self { host: Host::IpAddr(ip_addr), port: default_port });
		}
		...
		if !is_valid_hostname(hostname) {
			bail!("failed to parse address `{}`: hostname is invalid", host_addr)
		}
		Ok(Self { host: Host::Hostname(hostname.to_string()), port })
	}

	/// Resolves the host if necessary and returns a `SocketAddr`.
	pub async fn resolve(&self) -> anyhow::Result<SocketAddr> {
		self.host.resolve().await.map(|ip_addr| SocketAddr::new(ip_addr, self.port))
	}
...
fn _find_private_ip(interfaces: &[NetworkInterface]) -> Option<(String, IpAddr)> {
	...
	interfaces
		...
		.sorted_by_key(|(interface, ip_net)| {
			(ip_net.is_ipv6(), is_dormant(interface), std::cmp::Reverse(ip_net.prefix()))
		})
		.next()
...
fn is_forwardable_ip(ip_addr: &IpAddr) -> bool {
	...
	NON_FORWARDABLE_NETWORKS
        .get_or_init(|| {
            ...
            .iter()
            .map(|network| network.parse().expect("Failed to parse network range. This should never happen! Please, report on https://github.com/querent-ai/querent/issues."))
            .collect()
        })
        .iter()
...
fn _get_hostname(hostname: OsString) -> io::Result<String> {
	...
	if is_valid_hostname(&hostname_lossy) {
		...
		Err(io::Error::new(io::ErrorKind::Other, format!("invalid hostname: `{hostname_lossy}`")))
	}
}
...
pub fn is_valid_hostname(hostname: &str) -> bool {
	...
	if !hostname.chars().all(|ch| ch.is_ascii_alphanumeric() || ch == '-' || ch == '.') {
		return false;
	}
...
mod tests {
	...
	fn test_parse_host() {
		...
		assert_eq!("localhost".parse::<Host>().unwrap(), Host::Hostname("localhost".to_string()));
	}
	...
	fn test_parse_addr_helper(addr: &str, expected_addr_opt: Option<&str>) {
		...
		if let Some(expected_addr) = expected_addr_opt {
			assert!(addr_res.is_ok(), "Parsing `{addr}` was expected to succeed.");
	...
	fn test_is_valid_hostname() {
		for hostname in
			&["VaLiD-HoStNaMe", "50-name", "235235", "example.com", "VaLid.HoStNaMe", "123.456"]
		{
			assert!(is_valid_hostname(hostname), "Hostname `{hostname}` is valid.",);
		}
		...
		for hostname in &[
			...
			assert!(!is_valid_hostname(hostname), "Hostname `{hostname}` is invalid.");
		}
	}
	...
	fn test_is_forwardable_ip() {
		for ip in ["192.168.0.42", "172.16.0.42", "10.0.0.42"] {
			assert!(is_forwardable_ip(&ip.parse::<IpAddr>().unwrap()), "IP `{ip}` is forwardable!");
		}
	...
	fn test_is_private_ip() {
		for ip in ["192.168.0.42", "172.16.0.42", "10.0.0.42"] {
			assert!(is_private_ip(&ip.parse::<IpAddr>().unwrap()), "IP `{ip}` is private!");
		}
		for ip in ["192.169.0.42", "172.32.0.42", "11.0.0.42"] {
			assert!(!is_private_ip(&ip.parse::<IpAddr>().unwrap()), "IP `{ip}` is public!");
		}
	}

	#[test]
	fn test_get_hostname() {
		assert_eq!(get_hostname().unwrap(), hostname::get().unwrap().to_string_lossy().to_string());
