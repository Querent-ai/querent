...
use crate::{
	...
	Command, Handler, Health, MessageBus, Observation, Querent, Supervisable,
};
...
async fn test_actor_stops_when_last_messagebus_is_dropped() {
	...
	let (ping_recv_messagebus, ping_recv_handle) =
		querent.spawn_builder().spawn(PingReceiverActor::default());
...
async fn test_ping_actor() {
	...
	let (ping_sender_messagebus, ping_sender_handle) =
		querent.spawn_builder().spawn(PingerSenderActor::default());
	assert_eq!(
...
async fn test_timeouting_actor() {
	let querent = Querent::with_accelerated_time();
	let (buggy_messagebus, buggy_handle) = querent.spawn_builder().spawn(BuggyActor);
	...
	assert_eq!(buggy_handle.check_health(true), Health::Healthy);
	querent.sleep(crate::HEARTBEAT.mul(2)).await;
...
async fn test_actor_running_states() {
	...
	assert_eq!(*obs, 10);
	querent.sleep(Duration::from_millis(1)).await;
	assert!(ping_handle.state() == ActorState::Processing);
	querent.assert_quit().await;
}
...
async fn test_looping() -> anyhow::Result<()> {
	...
	let (looping_actor_messagebus, looping_actor_handle) =
		querent.spawn_builder().spawn(looping_actor);
...
async fn test_actor_spawning_actor() -> anyhow::Result<()> {
	let querent = Querent::with_accelerated_time();
	let (messagebus, handle) = querent.spawn_builder().spawn(SpawningActor::default());
...
async fn test_actor_finalize_error_set_exit_status_to_panicked() -> anyhow::Result<()> {
	let querent = Querent::with_accelerated_time();
	let (messagebus, handle) = querent.spawn_builder().spawn(BuggyFinalizeActor);
...
async fn test_actor_return_response() -> anyhow::Result<()> {
	...
	let (messagebus, _handle) = querent.spawn_builder().spawn(adder);
...
async fn test_drain_is_called() {
	...
	let (messagebus, handle) = querent.spawn_builder().spawn(test_actor_with_drain);
	assert_eq!(
...
async fn test_unsync_actor() {
	...
	let (messagebus, _handle) = querent.spawn_builder().spawn(unsync_message_actor);
