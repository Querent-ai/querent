...
impl<T> LockedOption<T> {
	pub fn none() -> Self {
		LockedOption { opt: Mutex::new(None), has_val: AtomicBool::new(false) }
	}
...
mod tests {
	...
	async fn test_try_recv_high_priority() -> anyhow::Result<()> {
		let (sender, receiver) = super::channel::<usize>(QueueCapacity::Unbounded);
		sender.send_low_priority(1).await?;
		assert_eq!(receiver.try_recv_high_priority_message(), Err(RecvError::NoMessageAvailable));
		Ok(())
	}
	...
	async fn test_recv_timeout_simple() -> anyhow::Result<()> {
		...
		assert!(matches!(receiver.try_recv(), Err(RecvError::NoMessageAvailable)));
		Ok(())
	}
	...
	async fn test_try_recv_high() {
		...
		assert_eq!(rx.try_recv_high_priority_message(), Err(RecvError::NoMessageAvailable));
		assert_eq!(rx.try_recv(), Ok(1));
		assert_eq!(rx.try_recv(), Err(RecvError::NoMessageAvailable));
	}
}
