...
enum SchedulerMessage {
	ProcessTime,
	Schedule { callback: Callback, timeout: Duration },
}
...
impl SchedulerClient {
	...
	fn is_advance_time_forbidden(&self) -> bool {
		self.inner.no_advance_time_guard_count.load(Ordering::SeqCst) > 0
	}
	...
	pub fn schedule_event<F: FnOnce() + Send + Sync + 'static>(
		...
		let _ = self
			.inner
			.tx
			.send(SchedulerMessage::Schedule { callback: Box::new(callback), timeout });
	}

	// Increases the number of reasons to not simulate advance time.
	pub(crate) fn inc_no_advance_time(&self) {
		self.inner.no_advance_time_guard_count.fetch_add(1, Ordering::SeqCst);
	}
	...
	pub(crate) fn dec_no_advance_time(&self) {
		let previous_count = self.inner.no_advance_time_guard_count.fetch_sub(1, Ordering::SeqCst);
		if previous_count == 1 {
			self.process_time();
		}
	}
...
impl Scheduler {
	...
	fn scheduler_client(&self) -> Option<SchedulerClient> {
		let scheduler_client = SchedulerClient { inner: self.weak_scheduler_client.upgrade()? };
...
impl Scheduler {
	...
	/// (if Querent::simulate_time_shift(..) has been called).
	...
	fn timeout_event(&mut self, deadline: Instant, callback: Callback) -> TimeoutEvent {
		...
		TimeoutEvent { deadline, event_id, callback }
	}
}
...
mod tests {
	...
	async fn test_scheduler_advance_time_fast_forward_initialize() {
		...
		querent.spawn_builder().spawn(simple_actor);
		assert_eq!(count.load(Ordering::SeqCst), 0);
		querent.sleep(Duration::from_millis(15)).await;
	...
	async fn test_scheduler_advance_time_fast_forward_scheduled_message() {
		...
		querent.spawn_builder().spawn(simple_actor);
		assert_eq!(count.load(Ordering::SeqCst), 0);
		querent.sleep(Duration::from_secs(10)).await;
