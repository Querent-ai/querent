...
use crate::{
	...
	registry::ActorJoinHandle, Actor, ActorContext, ActorExitStatus, Command, MessageBus,
	Observation,
};
...
impl<A: Actor> ActorHandle<A> {
	pub(crate) fn new(
		...
		ActorHandle { actor_context, last_state, join_handle }
	}
	...
	pub fn refresh_observe(&self) {
		let observation_already_enqueued =
			self.actor_context.set_observe_enqueued_and_return_previous();
	...
	async fn observe_with_priority(&self, priority: Priority) -> Observation<A::ObservableState> {
		...
		Observation { obs_type: ObservationType::PostMortem, state }
	}
	...
	pub fn pause(&self) {
		let _ = self.actor_context.messagebus().send_message_with_high_priority(Command::Pause);
	}

	/// Resumes a paused actor.
	pub fn resume(&self) {
		let _ = self.actor_context.messagebus().send_message_with_high_priority(Command::Resume);
	}
...
mod tests {
	...
	async fn test_panic_in_actor() -> anyhow::Result<()> {
		...
		let (messagebus, handle) = querent.spawn_builder().spawn(PanickingActor::default());
	...
	async fn test_exit() -> anyhow::Result<()> {
		let querent = Querent::with_accelerated_time();
		let (messagebus, handle) = querent.spawn_builder().spawn(ExitActor::default());
	...
	async fn test_observation_debounce() {
		// TODO investigate why Querent::with_accelerated_time() does not work here.
		let querent = Querent::new();
		let (_, actor_handle) = querent.spawn_builder().spawn(ObserveActor::default());
		for _ in 0..10 {
			actor_handle.refresh_observe();
			querent.sleep(Duration::from_millis(10)).await;
		}
