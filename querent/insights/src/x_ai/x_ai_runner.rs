use crate::{
	extract_sentences, prompts::get_suggestions_prompt,
...
use std::{
	collections::{HashMap, HashSet}, pin::Pin, sync::{Arc, RwLock}
};
            ...
            if *event_type == EventType::Vector {
                for storage in storages.iter() {
                    if query.is_empty() {
                        ...
                        let suggestions_response = self.llm.generate(&human_message).await.map_err(|e| {
                            InsightError::new(
                                InsightErrorKind::Internal,
                    ...
                    storage.insert_insight_knowledge(
