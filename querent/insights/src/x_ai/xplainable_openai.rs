use crate::{
	rerank_documents, ConfigCallbackResponse, CustomInsightOption, Insight, InsightConfig,
	InsightCustomOptionValue, InsightError, InsightErrorKind, InsightInfo, InsightInput,
		...
		for (event_type, storages) in self.config.event_storages.iter() {
			if *event_type == EventType::Vector {
				for storage in storages.iter() {
					println!("Running Auto suggest-------------------{:?}", query);
					if query.is_empty() {
						println!("Running Auto suggest-------------------");
						...
						let suggestions_prompt = format!(
							"The following are autogenerated queries are based on a user's private domain data stored in a semantic data fabric within a PostgreSQL table. \
							These queries have been generated by identifying the most frequent subject-object pairs and selecting sentences that contain these pairs. \
							This ensures that the suggestions are both relevant and contextually rich.\n\n{}\n\n\
							Based on these autogenerated queries, please provide different suggestions for questions to ask.",
...
Output:"
							...
							println!(
								"This the final message that goes-------------------------{:?}",
								human_message
							);
